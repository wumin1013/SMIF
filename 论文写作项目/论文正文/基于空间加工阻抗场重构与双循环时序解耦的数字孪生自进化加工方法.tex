...
\begin{abstract}
复杂CNC切削过程同时受材料非均匀性、毛坯余量偏差、刀具磨损与机床热漂移等多源不确定性影响，导致离线规划的静态机理模型与实际加工状态之间形成显著的 Sim-to-Real Gap。
为此，本文提出一种\emph{空间加工阻抗场}（Spatial Machining Impedance Field, SMIF）驱动的数字孪生自进化加工方法：以行程域（stroke domain）为统一坐标，将在线实测负载与刀具-工件啮合几何在同一空间基准中对齐，并将各类不可分辨的物理/几何扰动集总为位置相关的等效阻抗。
在此基础上，构建“毫秒级执行环 + 层间进化环”的双循环时序解耦机制：执行环通过参考功率跟踪实现安全加工并沉淀阻抗知识；进化环基于全场阻抗分布识别全局瓶颈与能力冗余，触发工艺结构重构与运动参数再规划，实现从被动自适应到主动自进化的跨越。
实验表明，该框架能够在不确定工况下逐层收敛仿真-现实鸿沟，并在保证功率硬约束的前提下提升加工效率与稳定性。
\end{abstract}

\begin{keyword}
数字孪生 \sep 智能加工 \sep 自进化 \sep 空间加工阻抗场 \sep 行程域 \sep 工艺重规划
\end{keyword}
...
\begin{figure}[htbp]
    ...
    \caption{双循环控制逻辑框图：执行环（RT-AFC）与进化环（Inter-layer Re-planning）}
    \label{fig:loops}
\end{figure}

\begin{enumerate}
    \item \textbf{执行环（Execution Loop, RT-AFC）}：毫秒级在线反馈控制。输入为当前PIT/参考功率，输出为进给倍率（override）与实测数据流；目标是在不超过 $P_{rated}$ 的前提下尽量跟踪参考利用率。
    \item \textbf{进化环（Evolutionary Loop, Inter-layer Re-planning）}：层间（inter-layer）重规划。输入为上一层沉淀的阻抗场与瓶颈集合，输出为更新后的层结构（$a_p/a_e$）与规划进给 $F_{plan}(s)$，并再生NC代码与PIT。
\end{enumerate}

为避免概念混乱，本文将“阻抗估计/更新”视为进化环的关键子模块：执行环负责采集，进化环负责沉淀与泛化。

\subsection{执行环：参考功率跟踪与稳态区间统计}
...
\end{equation}
对比 $\bar{P}_{meas}$ 与区间参考功率 $P_{target}$，更新下一轮的进给倍率：
\begin{equation}
R_{i,N+1}=R_{i,N}\left(1+\gamma\frac{P_{target}-\bar{P}_{meas}}{P_{target}}\right),
\end{equation}
其中 $\gamma$ 为迭代步长。该“run-to-run”更新机制能在保证稳定性的同时逐代逼近目标利用率。
