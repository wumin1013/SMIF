...
class MillingAnalysisTool:
    ...
    def create_data_processing_tab(self):
        ...
        # 工程应用：仅保留“MRR稳态区间最小行程长度”
        ttk.Label(param_frame, text="最小行程长度(mm):").grid(row=0, column=0, sticky="w")
        ttk.Entry(param_frame, textvariable=self.mrr_min_length, width=10).grid(row=0, column=1, padx=6, sticky="ew")
        ttk.Label(param_frame, text="(将MRR恒定的连续段划为稳态区间)", foreground="#666666").grid(
